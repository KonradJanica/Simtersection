...
public class BottomPanel extends JPanel implements ActionListener, ChangeListener {
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		if (src == bStartPause) {
			...
			final JFrame popbox = new JFrame("Quit");

	        int result = JOptionPane.showConfirmDialog(popbox, "Do you want to Exit ?", "Exit Confirmation : ", JOptionPane.YES_NO_OPTION);
	        if (result == JOptionPane.YES_OPTION){               
	        	popbox.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
	        	popbox.dispatchEvent(new WindowEvent(popbox, WindowEvent.WINDOW_CLOSING));
	        } else if (result == JOptionPane.NO_OPTION){   
	        	popbox.setDefaultCloseOperation(JFrame.DO_NOTHING_ON_CLOSE);
	        	popbox.dispatchEvent(new WindowEvent(popbox, WindowEvent.WINDOW_CLOSING));
	        }
		} else if(src == bTick) {
			sw.tickPressed();

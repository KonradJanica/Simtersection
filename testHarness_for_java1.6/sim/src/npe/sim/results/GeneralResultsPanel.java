...
public class GeneralResultsPanel extends JPanel {
	...
	private ArrayList<Boolean> overallData;
	...
	private void createResultsSummary()
	{
		...
		lineNumber += 2.5;		
		JLabel st = new JLabel("Test start time: " + startTime);
		...
		for (int i=0; i<overallData.size(); i++){
			lineNumber++;
			if (overallData.get(i) == true){
				result = "passed!";
			} else {
				result = "failed!";
			}
			JLabel total = new JLabel("Test " + i + ": " + result);
